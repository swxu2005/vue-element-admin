<template>
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="120px" @keyup.enter.native="dataFormSubmitHandle()">
      ...
      <el-form-item v-if="dataForm.pid === '0'" :label="$t('dict.dictType')" prop="dictType">
        <el-input v-model="dataForm.dictType" :placeholder="$t('dict.dictType')"/>
      </el-form-item>
      <el-form-item v-if="dataForm.pid !== '0'" :label="$t('dict.dictValue')" prop="dictValue">
        <el-input v-model="dataForm.dictValue" :placeholder="$t('dict.dictValue')"/>
      </el-form-item>
      <el-form-item :label="$t('dict.sort')" prop="sort">
        <el-input-number v-model="dataForm.sort" :min="0" :label="$t('dict.sort')" controls-position="right"/>
      </el-form-item>
